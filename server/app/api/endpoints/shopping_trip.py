...
async def create_shopping_trip(trip_data: ShoppingTripCreate):
    ...
    if not trip_id:
        raise HTTPException(status_code=500, detail="Échec de la création du shopping trip")
...
async def get_shopping_trip_for_user(trip_id: str, user_id: str):
    ...
    if not trip:
        raise HTTPException(status_code=404, detail="Shopping trip non trouvé pour cet utilisateur")
    return trip
...
async def delete_shopping_trip(trip_id: str, user_id: str):
    ...
    if not deleted:
        raise HTTPException(status_code=404, detail="Shopping trip non trouvé ou déjà supprimé")
